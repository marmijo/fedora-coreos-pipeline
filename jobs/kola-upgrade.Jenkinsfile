...
properties([
    ...
    parameters([
      ...
      string(name: 'TARGET_VERSION',
             ...
             defaultValue: '41.20250302.2.0', //again, for testing purposes
             trim: true),
      string(name: 'ARCH',
...
def start_versions = params.START_VERSIONS.trim().split('\\s+')
    ...
    try {
            ...
            start_versions.eachWithIndex { start_version, i ->
                ...
                catchError(buildResult: 'UNSTABLE') {
                    ...
                    pipeutils.withOptionalExistingCosaRemoteSession(
                        ...
                        stage_name = "BuildFetch " +  start_versions[i]
                        stage(stage_name) {
                            def commitopt = ''
                        ...
                        stage("Upgrade from " +  start_versions[i]) {
        ...
        """)
                ...
                if (currentBuild.result == 'ABORTED') {
                    echo "Detected manual job abort. Stopping immediately."
                    error("Pipeline manually aborted.")
                }
            } // end of start_versions loop
            // check if any of the start_versions[i] stages failed
        ...
        if (currentBuild.result == 'DONT SPAM SLACK TODAY') {
